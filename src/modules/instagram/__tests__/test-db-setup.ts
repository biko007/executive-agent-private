/**
 * Test DB Setup Helper — creates a fresh Postgres test database with insta tables.
 *
 * Usage:
...
export async function setupTestDb(): Promise<{ testDbName: string; cleanup: () => Promise<void> }> {
  ...
  const testDbName = `openclaw_test_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
