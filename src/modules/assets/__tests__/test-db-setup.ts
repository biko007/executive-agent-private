/**
 * Test DB Setup Helper — creates a fresh Postgres test database with assets tables (V022).
 *
 * Usage:
...
export async function setupTestDb(): Promise<{ testDbName: string; cleanup: () => Promise<void> }> {
  ...
  const testDbName = `openclaw_test_assets_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  ...
  `);
  const migrationPath = join(import.meta.dir, '../migrations/V022__assets_tables.sql');
