/**
 * Test DB Setup Helper — creates a fresh Postgres test database with fleet tables (V025).
 *
 * Usage:
...
export async function setupTestDb(): Promise<{ testDbName: string; cleanup: () => Promise<void> }> {
  ...
  const testDbName = `openclaw_test_fleet_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  ...
  // audit_log table (needed by store for audit.log calls)
  await testPool.query(`
  ...
  // Apply V025 fleet tables migration
  const migrationPath = join(import.meta.dir, '../migrations/V025__fleet_tables.sql');
