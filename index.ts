...
for (const k of days) {
  ...
  async function generateBriefingText(): Promise<string> {
    ...
    const parts: string[] = [`🧠 Briefing — ${fmtDT.format(now)}\n`];
    ...
    try {
      ...
      parts.push(
        `${icon} Wetter Tuttlingen: ${c.temperature_2m}°C (gefühlt ${c.apparent_temperature}°C), ` +
    ...
    } catch { parts.push('🌡 Wetter: nicht verfügbar'); }
    parts.push('');
    ...
    try {
      ...
      if (!ds.length) parts.push('   • keine offenen Drafts');
