...
import {
  insertConversationTurn, getLastConversationLogId, updateExtractStatus,
  getActiveOwnerFacts, rejectFact, listActiveFacts, getFactById,
} from './src/modules/memory/store.js';
import {
  resolveTranscript, shouldExtractMemory, extractFacts,
  runExtractSweep, invalidateRecallCache,
} from './src/modules/memory/extract.js';
import type { LlmCompleteFunc } from './src/modules/memory/extract.js';
...
import {
  ...
  createPendingTelegramBinding,
  extractTelegramIdentity,
  isBoundTelegramOwner,
  listActiveTelegramBindings,
  listActiveTelegramOwnerUserIds,
...
export default function (api: any) {
  ...
  g.__ea_voiceTranscriptStash ??= new Map<string, { text: string; ts: number }>();
  const voiceTranscriptStash: Map<string, { text: string; ts: number }> =
...
// ---- M365 config
  ...
  if (!telegramBotToken) {
    try {
      ...
      if (fs.existsSync(ocCfgPath)) {
        ...
        if (typeof raw === 'string') telegramBotToken = raw;
        else if (raw?.source === 'env' && raw?.id) telegramBotToken = process.env[raw.id] || '';
      }
    } catch { /* ignore */ }
  }
   ...
   */
  async function sendTelegramDocument(chatId: string, filePath: string, caption?: string): Promise<boolean> {
    if (!telegramBotToken) return false;
    try {
  ...
  async function getTelegramTargets(
    ...
    try {
      ...
      return [...new Set(bindings.map((b) => b.telegramChatId).filter((id): id is string => Boolean(id)))];
    } catch (e: any) {
  ...
  const telegramTargetCache: Record<TelegramBindingRole, string[]> = { operativ: [], dev: [] };

  async function refreshTelegramTargetCache(): Promise<void> {
  ...
  function getCachedTelegramTarget(roleTag: TelegramBindingRole, opts: { fallbackToOperativ?: boolean } = {}): string | undefined {
    const direct = telegramTargetCache[roleTag][0];
    if (direct) return direct;
    if (roleTag === 'dev' && opts.fallbackToOperativ) return telegramTargetCache.operativ[0];
    return undefined;
  }

  async function sendTelegramToRole(
    roleTag: TelegramBindingRole,
    text: string,
    opts: { fallbackToOperativ?: boolean } = {},
  ): Promise<boolean> {
  ...
  async function assertBoundOwner(ctxOrEvent: any): Promise<{ ok: true; chatId: string; userId: string } | { ok: false; chatId: string; userId: string }> {
  ...
  api.on('inbound_claim', (event: any, ctx: any) => {
    ...
    for (const [k, v] of voiceTranscriptStash) {
      if (Date.now() - v.ts > 300_000) voiceTranscriptStash.delete(k);
    }
    if (voiceTranscriptStash.size > 20) {
      const oldest = voiceTranscriptStash.keys().next().value;
      if (oldest) voiceTranscriptStash.delete(oldest);
    }
  }, { priority: 50 });
  ...
  api.on('before_prompt_build', async (event: any, ctx: any) => {
    ...
    if (ownerBinding.ok) {
      ...
      try {
        ...
        const allFacts: Array<{ id: number; fact: string; category: string | null; sensitivity: string }> =
          cache.facts ?? [];

        // Filter: normal always; sensitive only on keyword match
        const promptLower = prompt.toLowerCase();
        const filtered = allFacts.filter(f => {
          if (f.sensitivity === 'never_inject') return false;
          if (f.sensitivity === 'sensitive') {
            // Simple keyword check: category name must appear in prompt
            if (f.category && promptLower.includes(f.category)) return true;
            // Also check for health-related keywords
            if (f.category === 'health' && (
              promptLower.includes('gesundheit') || promptLower.includes('health') ||
        ...
        const injected: Array<{ id: number; fact: string }> = [];
        let totalBytes = 0;
        for (const f of filtered) {
          if (injected.length >= MAX_FACTS) break;
          const line = `- ${f.fact}`;
  ...
  const llmComplete: LlmCompleteFunc = (params) => api.runtime.llm.complete(params);
  ...
  api.registerCommand({
    ...
    handler: async (ctx: any) => {
      ...
      if (parts[0]?.toLowerCase() === 'create') {
        ...
        if (!guard.ok) return { text: 'Dieser Befehl ist nur fuer gebundene Owner verfuegbar.' };
        const role = parts[1] as TelegramBindingRole | undefined;
      ...
      const nonce = parts[0] || '';
      if (!nonce) return { text: 'Nutzung: /bind <nonce>' };
      ...
      try {
        ...
        if (e?.code === 'NONCE_MISMATCH') return { text: 'Binding fehlgeschlagen: Nonce ungueltig oder abgelaufen.' };
        if (e?.code === 'BINDING_CONFLICT') return { text: 'Binding fehlgeschlagen: Dieser Chat ist bereits gebunden.' };
      ...
      if (sub === 'drop') {
        ...
        if (chatId) {
          await sendTelegramWithKeyboard(
            chatId,
            `Fakt #${factId} entfernen?\n\n${factShort}\n\nFolge: status → rejected`,
            [[{ text: 'Ja, entfernen', callback_data: `memdrop_${factId}` }]],
          );
  ...
  api.registerCommand({
    ...
    handler: async (ctx: any) => {
      ...
      const reportFiles: { name: string; filePath: string; mtimeMs: number }[] = [];
      ...
      try {
        ...
        for (const name of entries) {
          ...
          if (st.isFile()) reportFiles.push({ name, filePath: fp, mtimeMs: st.mtimeMs });
        }
      } catch { /* dir missing */ }
      ...
      try {
        ...
        for (const name of homeEntries) {
          if (!name.startsWith('report-') || !name.endsWith('.md') || name.startsWith('.')) continue;
  ...
  api.on('message_received', async (event: any) => {
    try {
      ...
      if (memdropCb) {
        ...
        if (!chatId || !senderId) {
          if (senderId) await sendTelegram(senderId, 'Memory-Aktion abgelehnt: Telegram-Chat nicht verifizierbar.');
          return;
        }
  ...
  if (!g.__ea_mailScannerRegistered) {
    ...
    setInterval(async () => {
      try {
        ...
        if (!MAIL_BOOKING_SCAN_ENABLED) return;
  ...
  if (!g.__ea_memorySweepRegistered) {
    ...
    setInterval(async () => {
      try {
        const ownerIds = await listActiveTelegramOwnerUserIds();
        await runExtractSweep(
          llmComplete,
          ownerIds,
          { info: (m: string) => api.logger.info(m), warn: (m: string) => api.logger.warn(m) },
          (cid: string, txt: string) => sendTelegram(cid, txt),
          (await getTelegramTargets('operativ'))[0],
        );
      } catch (e: any) {
  ...
  if (!g.__ea_reportWatcherRegistered) {
    ...
    let reportDebounceSpec: ReturnType<typeof setTimeout> | null = null;
    try {
      fs.mkdirSync(REPORT_SPEC_DIR, { recursive: true });
      fs.watch(REPORT_SPEC_DIR, () => {
        if (reportDebounceSpec) clearTimeout(reportDebounceSpec);
    ...
    let reportDebounceHome: ReturnType<typeof setTimeout> | null = null;
    try {
      fs.watch(REPORT_HOME_DIR, (_event, filename) => {
        if (typeof filename === 'string' && filename.startsWith('report-') && filename.endsWith('.md')) {
          if (reportDebounceHome) clearTimeout(reportDebounceHome);
      ...
      try {
        ...
        try {
          paneContent = execSync(
            "tmux capture-pane -t bikosoc -p -l 5 2>/dev/null",
        ...
        if (contentHash === lastPaneContentHash) return;

        // Rate-limit
        const now = Date.now();
        if (now - lastWaitNotifyAt < WAIT_NOTIFY_COOLDOWN_MS) return;
  ...
  (async () => {
    ...
    try {
      ...
      if (tgBindingApplied > 0) api.logger.info(`[telegram-binding] Applied ${tgBindingApplied} migration(s)`);
