...
/draftshow ${id}` };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                ...
                if (args.length < 4)
                    return { text: '❌ Verwendung: /fleetadd <car|bike> <Hersteller> <Modell...> <Baujahr>' };
                const type = args[0].toLowerCase();
                if (type !== 'car' && type !== 'bike')
                    return { text: '❌ Typ muss "car" oder "bike" sein.' };
                const make = args[1];
                const yearStr = args[args.length - 1];
                const year = parseInt(yearStr, 10);
                if (!/^\d{4}$/.test(yearStr) || year < 1900 || year > 2100)
                    return { text: '❌ Ungültiges Baujahr (4-stellige Zahl erwartet).' };
                const model = args.slice(2, -1).join(' ');
                if (!model)
                    return { text: '❌ Modell fehlt.' };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                const id = String(ctx.args || '').trim();
                if (!id)
                    return { text: '❌ Verwendung: /fleetshow <id>' };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                ...
                if (parts.length < 3)
                    return { text: '❌ Verwendung: /fleetedit <id> <feld> <wert>' };
                ...
                switch (field.toLowerCase()) {
                    ...
                    case 'mileage': {
                        ...
                        if (isNaN(km))
                            return { text: '❌ km-Stand muss eine Zahl sein.' };
                        updates.mileage = km;
                        break;
                    }
                    case 'tuev': {
                        ...
                        if (!iso)
                            return { text: '❌ Datum im Format DD.MM.YYYY erwartet.' };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                const id = String(ctx.args || '').trim();
                if (!id)
                    return { text: '❌ Verwendung: /fleetdel <id>' };
                const v = getVehicle(id);
                if (!v)
                    return { text: `❌ Fahrzeug nicht gefunden: ${id}` };
                deleteVehicle(id);
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                ...
                const parts = raw.split(/\s+/);
                if (parts.length < 2)
                    return { text: '❌ Verwendung: /fleetservice <id> <typ> [kosten] [notiz]' };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                ...
                const parts = raw.split(/\s+/);
                if (parts.length < 3)
                    return { text: '❌ Verwendung: /fleetinsurance <id> <anbieter> <typ> [kosten/jahr]' };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                ...
                const parts = raw.split(/\s+/);
                if (parts.length < 2)
                    return { text: '❌ Verwendung: /fleettuev <id> <DD.MM.YYYY>' };
    ...
    api.registerCommand({
        ...
        handler: async (ctx) => {
            try {
                const id = String(ctx.args || '').trim();
                if (!id)
                    return { text: '❌ Verwendung: /fleetdocs <id>' };
                const v = getVehicle(id);
                if (!v)
                    return { text: `❌ Fahrzeug nicht gefunden: ${id}` };
                if (!v.documents.length)
                    return { text: `📎 Keine Dokumente für **${v.name}**.` };
    ...
    function addBookingAsSegment(tripId, booking) {
